...
export function ComputeForm({
  ...
  const num = (name: keyof FormValues) => register(name, { valueAsNumber: true });
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {sssLoan && (
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Balance {formatPHP(sssLoan.current_balance)}
                      </p>
                    </div>
                  )}
                  {pagibigLoan && (
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Balance {formatPHP(pagibigLoan.current_balance)}
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  {advances.map((a) => (
                    <div key={a.id} className="flex items-center gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Balance {formatPHP(a.current_balance)}
                        </p>
                      </div>
                      <div className="w-40">
                        <MoneyInput
                          aria-label={`Deduct from ${a.label ?? "advance"}`}
                          {...register(`advances.${a.id}` as const, { valueAsNumber: true })}
                        />
                      </div>
                    </div>
                  ))}
      ...
      <div className="space-y-4 lg:sticky lg:top-20 lg:self-start">
        <Card>
          ...
          <CardContent className="space-y-2">
            <Line label={`Wage (${values.days_worked || 0} days)`} value={formatPHP(row.weekly_salary - row.total_food_allowance - row.total_sleep_allowance)} />
            <Line label="Food allowance" value={formatPHP(row.total_food_allowance)} />
            <Line label="Sleep allowance" value={formatPHP(row.total_sleep_allowance)} />
            <Line label="Overtime" value={formatPHP(row.overtime_amount)} />
            <div className="my-2 border-t" />
            <Line label="Gross weekly" value={formatPHP(row.gross_weekly_salary)} strong />
            <div className="my-2 border-t" />
            <Line label="Contributions" value={`− ${formatPHP(row.sss_contribution + row.pagibig_contribution + row.philhealth_contribution)}`} />
            <Line label="Loan repayments" value={`− ${formatPHP(row.sss_loan_payment + row.pagibig_loan_payment)}`} />
            <Line label="Advances" value={`− ${formatPHP(row.total_advance_deduction)}`} />
            <Line label="Total deductions" value={`− ${formatPHP(row.total_deductions)}`} strong />
            <div className="my-2 border-t" />
            <div
              className={cn(
                "flex items-center justify-between rounded-xl px-3 py-2.5 text-lg font-bold",
                result.isNetNonPositive
                  ? "bg-destructive/10 text-destructive"
                  : "bg-success/10 text-success"
              )}
            >
              <span>Net pay</span>
              <span className="tabular-nums">{formatPHP(row.net_weekly_pay)}</span>
            </div>
            {result.isNetNonPositive && (
        ...
        <div className="flex items-center justify-between text-sm">
          {stepper.prevHref ? (
            <Link href={stepper.prevHref} className={cn(buttonVariants({ variant: "ghost", size: "sm" }))}>
          ...
          {stepper.nextHref ? (
            <Link href={stepper.nextHref} className={cn(buttonVariants({ variant: "ghost", size: "sm" }))}>
